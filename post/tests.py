...
class PostTestCase(APITestCase):
    def setUp(self):
        ...
        Friend.objects.create(f1Id=self.user2, f2Id=self.user1, status="A", isCopy=True)
        Friend.objects.create(
    ...
    def test_create_post(self):
        ...
        response = self.client.post(
            "/posts", request_body, HTTP_AUTHORIZATION="Token " + self.token4.key,
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

        response = self.client.post("/posts", request_body,)
    ...
    def test_get_post(self):
        ...
        response = self.client.get(f"/author/posts")
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        response = self.client.get(f"/author/{str(self.user1.id)}/posts")
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        response = self.client.get(f"/posts/{str(self.post2.id)}",)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

        response = self.client.get(f"/posts/{str(self.post3.id)}",)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

        response = self.client.get(
            f"/posts/{str(self.post2.id)}",
            HTTP_AUTHORIZATION="Token " + self.token1.key,
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        response = self.client.get(
            f"/posts/{str(self.post3.id)}",
            HTTP_AUTHORIZATION="Token " + self.token1.key,
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
